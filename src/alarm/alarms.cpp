...
static alarm_s current_alarm; // Data for the alarm we are currently editing
...
static void itemLoader(byte num) // 在menu_draw中遍历执行itemLoader
{
    if (num < ALARM_COUNT)
    {
        ...
        {
            memcpy(&alarm2, &current_alarm, sizeof(alarm_s));
        }
...
static void selectAlarm()
{
    ...
    setMenuFuncs(alarmsUp, mSelect, alarmsDown, itemLoader);
    menuData.func.draw = alarmsDraw;

    switch (setting.now)
    {
    case SETTING_NOW_NONE:
        alarm_get(menuData.selected, &current_alarm);
        setting.now = SETTING_NOW_EN;
        setting.val = current_alarm.enabled;
        break;

    case SETTING_NOW_EN:
        current_alarm.enabled = setting.val;
        setting.now = SETTING_NOW_HOUR;

        time_s time;
        time.hour = current_alarm.hour;
        time.ampm = CHAR_24;
        time_timeMode(&time, appConfig.timeMode);

        setting.val = time.hour;

        wasPM = (current_alarm.hour >= 12);
    ...
    case SETTING_NOW_HOUR:
    {
        ...
        setting.val = current_alarm.min;
    }
    break;

    case SETTING_NOW_MIN:
        current_alarm.min = setting.val;
    ...
    case SETTING_NOW_DAY_SUN:
        ...
        dayBit++;
        setting.val = (bool)(current_alarm.days & _BV(dayBit));
        ...
        {
            alarm_save(menuData.selected, &current_alarm);
