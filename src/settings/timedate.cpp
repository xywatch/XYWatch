...
{
    setMenuFuncs(timeDataUp, mSelect, timeDataDown, itemLoader);
...
static void selectDate()
{
    ...
    default: // Also SETTING_NOW_YEAR
        ...
        timeDateSet.date.day = time_calcWeekDay(timeDateSet.date.year, timeDateSet.date.month, timeDateSet.date.date);

        // Serial.printf("timeDateSet.date.day: %d, %d-%d-%d\n", timeDateSet.date.day, timeDateSet.date.year, timeDateSet.date.month, timeDateSet.date.date);
        endSelect();
        break;
    }
}
