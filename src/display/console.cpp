...
static uint8_t console_line_index = 0;
static uint8_t last_str_length = 0;
uint8_t now_str_length;

uint8_t show_last_line_ldx = 0; // 偏移行数，控制该变量可以看历史行,
...
bool up_line(void)
{
    ...
    if (show_last_line_ldx >= LINE_BUF_MAX - LINE_SHOW_CNT)
    { // 最大偏移行是20-8，再偏移第一行就显示到最后一行了
...
void console_log(unsigned short time_delay, const char *fmt, ...)
{
    ...
    if (this_line_char_cnt != -1)
    {
        ...
        {
            div_flag = 1;
            strcopy_cnt(string_buff[console_line_index].str_buff, firststr_buff, this_line_char_cnt);
        ...
        { // LINE_CHAR_CNT字符内有\n,\n在结尾或者在第一个
            ...
            {
                strcopy_cnt(string_buff[console_line_index].str_buff, " ", 1);
            }
            else
            {
                strcopy_cnt(string_buff[console_line_index].str_buff, firststr_buff, this_line_char_cnt);
    ...
    if (now_str_length < last_str_length)
    {
        ...
        {
            strcat(string_buff[console_line_index].str_buff, " "); // 加入空格清空上一行的显存
        }
    }
    ...
    if (console_line_index >= LINE_SHOW_CNT)
    { // 大于要显示的行数，就要显示新行
        new_line_flag = 1;
    }
    ...
    else
    {
        ...
        char show_index = (LINE_BUF_MAX + console_line_index - LINE_SHOW_CNT) % LINE_BUF_MAX; // 定位最后一行
