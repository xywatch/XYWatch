...
static u8 last_direction = UPT_MOVE_NONE;
...
static display_t draw()
{
    ...
    u8 direction = bma423_get_direction();

    uptMove = UPT_MOVE_NONE; // 还原

    // 向下/上然后迅速放平才算, 不灵敏

    // 放平了, 看看上次是什么
    if (direction == DIRECTION_DISP_UP) {
        uptMove = last_direction;
        last_direction = UPT_MOVE_NONE;
    }

    // 向下
    else if (direction == DIRECTION_TOP_EDGE)
    {
        last_direction = UPT_MOVE_DOWN;
    }
    // 向上
    else if (direction == DIRECTION_BOTTOM_EDGE)
    {
        last_direction = UPT_MOVE_UP;
    }
    else {
        last_direction = UPT_MOVE_NONE;
    }
