...
uint16_t bma4_write_config_file(struct bma4_dev *dev) {
  ...
  /* Disable advanced power save 1. 禁用高级省电模式 */
  ...
  if (rslt == BMA4_OK) {
    /* Disable config loading 2. 禁用配置加载*/
    rslt |= bma4_write_regs(BMA4_INIT_CTRL_ADDR, &config_load, 1, dev);
    /* Write the config stream 写入配置流 */
    ...
    /* Enable config loading and FIFO mode 启用配置加载 */
    config_load = 0x01;
    rslt |= bma4_write_regs(BMA4_INIT_CTRL_ADDR, &config_load, 1, dev);
    /* Wait till ASIC is initialized. Refer the data-sheet for
    more information 等待 ASIC 初始化 */
    dev->delay(150);
    ...
    if (config_stream_status != BMA4_ASIC_INITIALIZED) {
      ...
      /* Enable advanced power save  重新启用高级省电模式 */
  ...
  if (rslt == BMA4_OK) {
    ...
    if (rslt == BMA4_OK) {
      rslt |= write_regs(BMA4_FEATURE_CONFIG_ADDR, (uint8_t *)stream_data,
                         dev->read_write_len, dev);
      // 写入成功后，读取配置
      // uint8_t stream_data2[8];
      // bma4_read_regs(BMA4_FEATURE_CONFIG_ADDR, stream_data2,
      //                        dev->read_write_len, dev);
      // Serial.printf("index: %d\n[", index);
      // for (int i = 0; i < dev->read_write_len; i++) {
      //     if (i != 0) {
      //         Serial.printf(", "); // 在元素间添加逗号和空格
      //     }
      //     Serial.printf("%d:%d", stream_data[i], stream_data2[i]);
      // }
      // Serial.printf("]\n"); // 结束数组表示，并换行
    } else {
      Serial.printf("stream_transfer_write error2\n");
    }
  } else {
    Serial.printf("stream_transfer_write error1\n");
  }
