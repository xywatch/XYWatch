...
bool BMA423::begin(bma4_com_fptr_t readCallBlack,
  ...
  if (bma423_write_config_file(&__devFptr) != BMA4_OK) {
    ...
    DEBUG("BMA423 Write Config OK\n");
  }

  // 5. 检查 ASIC 状态
  uint8_t config_stream_status;
  bma4_read_regs(BMA4_INTERNAL_STAT, &config_stream_status, 1, &__devFptr);
  if (config_stream_status != BMA4_ASIC_INITIALIZED) {
    DEBUG("ASIC not initialized properly\n");
    return false;
  } else {
    DEBUG("ASIC initialized properly\n");
  }
...
uint32_t BMA423::getCounter() {
  ...
  Serial.println("bma423_step_counter_output error");
  return 0;
}
...
uint16_t BMA423::getInterruptStatus() {
